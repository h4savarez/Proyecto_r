...
ggplot(monthly, aes(x=proportion_death, y=date, colors= births)) +
geom_line()

```
```{r}
ggplot(monthly, aes(x=proportion_death, y= date)) +
  geom_line() +
...
monthly
ggplot(monthly, aes(x=log10(proportion_death) , y= ( (date)), color=handwasing_started)) +
  geom_point()+
   labs(title = "Efecto de lavado de manos con relacion a muertes") +
